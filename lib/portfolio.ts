...
export function validateHoldings(rows:Holding[]){if(rows.length>5000)throw Error('Too many holdings.');const seen=new Set<string>();for(const h of rows){if(!h.id||!h.name.trim()||!h.amc.trim()||!h.folio.trim())throw Error('Each holding needs a scheme name, AMC and folio.');if(!Number.isFinite(h.units)||h.units<0||!Number.isFinite(h.nav)||h.nav<0||![h.invested,h.value].every(n=>Number.isSafeInteger(n)&&n>=0))throw Error('Holding values must be valid and non-negative.');const key=(h.schemeId||nameKey(h.name))+'|'+amcKey(h.amc)+'|'+h.folio;if(seen.has(key))throw Error('Duplicate scheme and folio.');seen.add(key)}}
